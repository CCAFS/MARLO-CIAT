  ...
  <div class="history-mode text-center animated flipInX">
    ...
    [#else]
      <p>[@s.text name="programImpact.message.historyVersion" ]  
          [@s.param]<span>${output.modifiedBy.composedName?html}</span>[/@s.param]
          [@s.param]<span>${output.activeSince?datetime}</span>[/@s.param]
          [@s.param]<a href="[@s.url][@s.param name="outputID" value=outputID /][@s.param name="edit" value="true"/][/@s.url]">here</a>[/@s.param]
         [/@s.text]
      </p>
    [/#if]
  </div>
