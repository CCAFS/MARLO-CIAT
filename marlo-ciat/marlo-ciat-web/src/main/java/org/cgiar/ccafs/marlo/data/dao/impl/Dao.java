...
public class Dao<T> implements IDao<T> {
	...
	@Inject
	protected Dao(TypeLiteral<T> type, Provider<EntityManager> emp,
	...
	public <K> List<T> find(SingularAttribute<T, K> property, K value) {
		...
		return find(property, value, Optional.<Integer> absent(),
				Optional.<Integer> absent(), Optional.<Order> absent());
	}
	...
	public <K> List<T> find(SingularAttribute<T, K> property, K value,
		...
		Map<SingularAttribute<T, K>, K> properties = ImmutableMap
				.<SingularAttribute<T, K>, K> builder().put(property, value)
				.build();
	...
	private <Y> TypedQuery<T> getTypedQuery(
			Map<SingularAttribute<T, Y>, Y> properties, Optional<Order> order) {
	...
	private void setQueryLimits(Query q, Optional<Integer> offset,
