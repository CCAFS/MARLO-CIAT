...
public class BaseAction extends ActionSupport implements Preparable,
	...
	public boolean canAccessSuperAdmin() {
		return this.securityContext
				.hasAllPermissions(Permission.FULL_PRIVILEGES);
	}
	...
	public void clearPermissionsCache() {
		((APCustomRealm) securityContext.getRealm())
				.clearCachedAuthorizationInfo(securityContext.getSubject()
						.getPrincipals());
	}
	...
	public Long getCrpID() {
		if (session != null && !session.isEmpty()) {
			try {
				Crp crp = (Crp) session.get(APConstants.SESSION_CRP) != null ? (Crp) session
						.get(APConstants.SESSION_CRP) : null;
	...
	public User getCurrentUser() {
		...
		if (session != null && !session.isEmpty()) {
			try {
				u = session.get(APConstants.SESSION_USER) != null ? (User) session
						.get(APConstants.SESSION_USER) : null;
			} catch (Exception e) {
	...
	/*public List<Auditlog> getListLog(IAuditLog object) {
		try {
			return auditLogManager.listLogs(object.getClass(),
					Long.parseLong(object.getId().toString()),
					this.getActionName());
		} catch (Exception e) {
...
*/
	...
	public int getPlanningYear() {
		return Integer.parseInt(this.getSession()
				.get(APConstants.CRP_PLANNING_YEAR).toString());
	...
	public String getVersion() {
		...
		if (version == null) {
			...
			try {
				prop.load(ServletActionContext.getServletContext()
	...
	public boolean hasPermission(String fieldName) {
		if (basePermission == null) {
			...
			return securityContext.hasPermission(this.getBasePermission() + ":"
					+ fieldName);
		}
	}
	...
	public boolean isLessonsActive() {
		return Integer.parseInt(this.getSession()
				.get(APConstants.CRP_LESSONS_ACTIVE).toString()) == 1;
	}
	...
	public boolean isPlanningActive() {
		return Integer.parseInt(this.getSession()
				.get(APConstants.CRP_PLANNING_ACTIVE).toString()) == 1;
