...
public class V1_0_1_20160622_0920__DeleteTriggers implements JdbcMigration {

	private static Logger LOG = LoggerFactory
			.getLogger(V1_0_1_20160622_0920__DeleteTriggers.class);

	@Override
	public void migrate(Connection connection) throws Exception {
		Statement statement = connection.createStatement();
		String[] tableNames = { "crp_users", "crps", "users" };
		...
		LogDatabaseManager dbManager = new LogDatabaseManager(connection,
				dbName);
		...
		try {
			...
			for (String tableName : tableNames) {
				try {
					...
					triggerManager = new LogTriggersManager(connection, dbName,
							tableName);

					triggerManager.deleteTrigger("insert");
					triggerManager.deleteTrigger("update");
				} catch (Exception e) {
					throw e;
